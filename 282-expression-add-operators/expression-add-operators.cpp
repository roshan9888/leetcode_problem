...
//             // cout<<number<<" ";
//             if(ind==0){
//                 solve(ind+1,temp+num,number,number,num,target,ans);
//             }
//             else{
//                 solve(ind+1,temp+"+"+num.substr(ind,i-ind+1),sum+number,number,num,target,ans);
//                 solve(ind+1,temp+"-"+num.substr(ind,i-ind+1),sum-number,-number,num,target,ans);
//                 // solve(ind+1,temp+"*"+num.substr(ind,i-ind+1),(sum-prev+(prev*number)),(prev*number),num,target,ans);
//                 solve(ind + 1, temp + "*" + num.substr(ind, i - ind + 1),
//       sum - prev + (prev * number), (prev * number), num, target, ans);
//             }
//         }
//     }
